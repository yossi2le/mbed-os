...
int TDBStore::garbage_collection()
{
    ...
    while (reserved_size) {
        ...
        ret = read_area(_active_area, to_offset, chunk_size, _work_buf + to_offset);
        if (ret) {
            return ret;
        }
        ret = write_area(1 - _active_area, to_offset, chunk_size, _work_buf + to_offset);
        if (ret) {
            return ret;
        }
        to_offset += chunk_size;
...
int TDBStore::init()
{
    ...
    if ((uint32_t)_bd->size() != _bd->size()) {
        MBED_ERROR(MBED_ERROR_INVALID_SIZE, "Underlying BD size should not exceed 32 bits");
    }
    ...
    for (uint8_t area = 0; area < _num_areas; area++) {
        ...
        if (ret == MBED_ERROR_INVALID_DATA_DETECTED) {
            bool erased;
            uint32_t erase_unit_num = _master_record_offset / _bd->get_erase_size(_master_record_offset);
            if (is_erase_unit_erased(area, erase_unit_num, erased)) {
    ...
    if ((area_state[0] == TDBSTORE_AREA_STATE_VALID) && (area_state[1] == TDBSTORE_AREA_STATE_VALID)) {
        ...
        ret = erase_erase_unit(1 - _active_area, 0);
        if (ret) {
            MBED_ERROR(ret, "TDBSTORE: Unable to erase media start at init");
        }
    }
...
int TDBStore::reset_area(uint8_t area)
{
    ...
    do {
        ret = erase_erase_unit(area, bd_offset);
        if (ret) {
            return ret;
        }
...
int TDBStore::reserved_data_set(const void *reserved_data, size_t reserved_data_buf_size)
{
    ...
    while (check_size) {
        ...
        ret = read_area(_active_area, offset, chunk_size, _work_buf + offset);
        if (ret) {
            goto end;
        }
...
int TDBStore::reserved_data_get(void *reserved_data, size_t reserved_data_buf_size)
{
    ...
    int ret = read_area(_active_area, 0, reserved_data_buf_size, reserved_data);
