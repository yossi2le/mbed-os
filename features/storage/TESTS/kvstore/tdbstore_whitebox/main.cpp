...
SDBlockDevice bd(MBED_CONF_SPIF_DRIVER_SPI_MOSI, MBED_CONF_SPIF_DRIVER_SPI_MISO,
                    MBED_CONF_SPIF_DRIVER_SPI_CLK, MBED_CONF_SPIF_DRIVER_SPI_CS);
...
#else
// Satisfy compiler
HeapBlockDevice bd(4096, 1, 1, 128);
FlashSimBlockDevice flash_bd(&bd);
#endif
...
static void white_box_test()
{
    ...
    for (int bd_num = 0; bd_num < num_bds; bd_num++) {
        ...
        TDBStore *kvs = new TDBStore(test_bd);
        ...
        result = kvs->reserved_data_set(res_val1, strlen(res_val1));
        TEST_ASSERT_EQUAL(MBED_SUCCESS, result);

        result = kvs->reserved_data_set(res_val2, strlen(res_val2));
        TEST_ASSERT_EQUAL(MBED_ERROR_WRITE_FAILED, result);

        result = kvs->set(key1, key1_val1, strlen(key1_val1), 0);
        TEST_ASSERT_EQUAL(MBED_SUCCESS, result);

        result = kvs->set(key2, key2_val1, strlen(key2_val1), 0);
        TEST_ASSERT_EQUAL(MBED_SUCCESS, result);

        result = kvs->set(key2, key2_val2, strlen(key2_val2), 0);
        TEST_ASSERT_EQUAL(MBED_SUCCESS, result);
        timer.reset();
        result = kvs->set(key2, key2_val3, strlen(key2_val3), 0);
        elapsed = timer.read_ms();
        printf("Elapsed time for set is %d ms\n", elapsed);

        TEST_ASSERT_EQUAL(MBED_SUCCESS, result);

        result = kvs->set(key3, key3_val1, strlen(key3_val1), 0);
        TEST_ASSERT_EQUAL(MBED_SUCCESS, result);

        result = kvs->get(key3, get_buf, sizeof(get_buf), &actual_data_size);
        TEST_ASSERT_EQUAL(MBED_SUCCESS, result);
        TEST_ASSERT_EQUAL(strlen(key3_val1), actual_data_size);
        TEST_ASSERT_EQUAL_STRING_LEN(key3_val1, get_buf, strlen(key3_val1));

        for (int j = 0; j < 2; j++) {
            result = kvs->set(key4, key4_val1, strlen(key4_val1), 0);
            TEST_ASSERT_EQUAL(MBED_SUCCESS, result);

            result = kvs->set(key4, key4_val2, strlen(key4_val2), 0);
            TEST_ASSERT_EQUAL(MBED_SUCCESS, result);
        }

        result = kvs->remove(key3);
        TEST_ASSERT_EQUAL(MBED_SUCCESS, result);

        result = kvs->remove(key3);
        TEST_ASSERT_EQUAL(MBED_ERROR_ITEM_NOT_FOUND, result);

        result = kvs->get_info(key5, &info);
        TEST_ASSERT_EQUAL(MBED_ERROR_ITEM_NOT_FOUND, result);

        result = kvs->set(key5, key5_val1, strlen(key5_val1), KVStore::WRITE_ONCE_FLAG);
        TEST_ASSERT_EQUAL(MBED_SUCCESS, result);

        result = kvs->set(key5, key5_val2, strlen(key5_val2), 0);
        TEST_ASSERT_EQUAL(MBED_ERROR_WRITE_PROTECTED, result);

        result = kvs->remove(key5);
        TEST_ASSERT_EQUAL(MBED_ERROR_WRITE_PROTECTED, result);
        ...
        result = kvs->get(key5, get_buf, sizeof(get_buf), &actual_data_size);
        TEST_ASSERT_EQUAL(MBED_SUCCESS, result);
        ...
        for (int i = 0; i < 2; i++) {
            ...
            result = kvs->get(key1, get_buf, sizeof(get_buf), &actual_data_size);
            TEST_ASSERT_EQUAL(MBED_SUCCESS, result);
            TEST_ASSERT_EQUAL(strlen(key1_val1), actual_data_size);
            TEST_ASSERT_EQUAL_STRING_LEN(key1_val1, get_buf, strlen(key1_val1));

            timer.reset();
            result = kvs->get(key2, get_buf, sizeof(get_buf), &actual_data_size);
            elapsed = timer.read_ms();
            ...
            result = kvs->get(key4, get_buf, sizeof(get_buf), &actual_data_size);
            TEST_ASSERT_EQUAL(MBED_SUCCESS, result);
            ...
            result = kvs->iterator_open(&it, "This");
            TEST_ASSERT_EQUAL(MBED_SUCCESS, result);

            result = kvs->iterator_next(it, char_get_buf, sizeof(get_buf));
            TEST_ASSERT_EQUAL(MBED_SUCCESS, result);
            ...
            result = kvs->iterator_next(it, (char *)get_buf, sizeof(get_buf));
            TEST_ASSERT_EQUAL(MBED_ERROR_ITEM_NOT_FOUND, result);

            result = kvs->iterator_close(it);
            TEST_ASSERT_EQUAL(MBED_SUCCESS, result);

            result = kvs->reserved_data_get(get_buf, strlen(res_val1));
            TEST_ASSERT_EQUAL(MBED_SUCCESS, result);
...
static void error_inject_test()
{
    ...
    flash_bd.init();
    flash_bd.erase(0, num_blocks * block_size);
    flash_bd.deinit();
    ...
    for (uint8_t get_iter = 0; get_iter < 2; get_iter++) {
        ...
        result = kvs->get(key, get_buf, data_size, &actual_data_size);
        TEST_ASSERT_EQUAL(MBED_ERROR_INVALID_DATA_DETECTED, result);
...
Case cases[] = {
    Case("KVStore: White box test",     white_box_test,    greentea_failure_handler),
    Case("KVStore: Multiple set test",  multi_set_test,    greentea_failure_handler),
    Case("KVStore: Error inject test",  error_inject_test,   greentea_failure_handler),
};
